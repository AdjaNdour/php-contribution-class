...
<title>Ajouter une Semaine — Contribution Class</title>
  ...
  <div class="flex-1 flex flex-col min-w-0">
    ...
    <main class="flex-1 overflow-y-auto p-6 space-y-6">
      ...
      <div class="max-w-2xl mx-auto bg-surface border border-gray-100 rounded-xl shadow-sm p-8">
        <h2 class="font-display font-semibold text-lg text-text mb-6">Ajouter une cotisation hebdomadaire (Semaine)</h2>
        ...
        <form action="index.php?controller=gerant&action=addSemaine" method="POST" class="space-y-5">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label class="block text-xs font-semibold text-text-muted mb-2">NUMÉRO DE LA SEMAINE</label>
              <input type="number" name="numero" placeholder="Ex: 15" min="1" required class="w-full px-4 py-3 rounded-lg border border-gray-300 bg-background text-sm focus:ring-2 focus:ring-primary">
            </div>
            <div>
              <label class="block text-xs font-semibold text-text-muted mb-2">MONTANT REQUIS PAR APPRENANT (FCFA)</label>
              <input type="number" name="montantDemande" placeholder="Ex: 100" min="1" required class="w-full px-4 py-3 rounded-lg border border-gray-300 bg-background text-sm focus:ring-2 focus:ring-primary">
            </div>
          </div>
